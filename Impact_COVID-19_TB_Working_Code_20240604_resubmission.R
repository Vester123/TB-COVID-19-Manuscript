...
jpeg("forest_success2020_20240604.jpeg", width = 3*1500, height = 3*2000, res=3*100)
forest(x=meta_succ2020_2, print.subgroup.name=FALSE,text.random="Overall summary",text.random.w="Regional summary",ref=1,at=(c(0.5, 1, 2)),
             leftcols=c("studlab"),rightcols=c("effect","ci"),weight.study="same",weight.subgroup="same",test.subgroup.random=FALSE,xlim=c(0.5,2),
...
jpeg("forest_success2021_20240604.jpeg", width = 3*1500, height = 3*2000, res=3*100)
forest(x=meta_succ2021_2, print.subgroup.name=FALSE,text.random="Overall summary",text.random.w="Regional summary",ref=1,at=(c(0.5, 1, 2)),
       leftcols=c("studlab"),rightcols=c("effect","ci"),weight.study="same",weight.subgroup="same",test.subgroup.random=FALSE,xlim=c(0.5,2),
       lty.random = 0,sortvar=-TE,leftlabs = c("Country"),smlab=("Observed:Expected (Success)"),rightlabs = c("Risk Ratio","95% CI"),
       col.study="black",col.diamond = "white",col.diamond.lines ="black",col.square="black",col.inside="black",squaresize=0.5,col.by="black",allstudies=FALSE)
dev.off()
...
jpeg("forest_fail2020_20240604.jpeg", width = 3*1500, height = 3*2000, res=3*100)
...
jpeg("forest_fail2021_20240604.jpeg", width = 3*1500, height = 3*2000, res=3*100)
...
jpeg("forest_died2020_20240604.jpeg", width = 3*1500, height = 3*2000, res=3*100)
...
jpeg("forest_died2021_20240604.jpeg", width = 3*1500, height = 3*2000, res=3*100)
...
jpeg("forest_lost2020_20240604.jpeg", width = 3*1500, height = 3*2000, res=3*100)
...
jpeg("forest_lost2021_20240604.jpeg", width = 3*1500, height = 3*2000, res=3*100)
...
tab <- data.frame(Success = c(sum(dat_2012$newrel_succ, na.rm = T), sum(dat_2013$newrel_succ, na.rm = T), sum(dat_2014$newrel_succ, na.rm = T), sum(dat_2015$newrel_succ, na.rm = T), sum(dat_2016$newrel_succ, na.rm = T), sum(dat_2017$newrel_succ, na.rm = T), sum(dat_2018$newrel_succ, na.rm = T), sum(dat_2019$newrel_succ, na.rm = T), sum(dat_2020$newrel_succ, na.rm = T), sum(dat_2021$newrel_succ, na.rm = T)),
                  Failure = c(sum(dat_2012$newrel_fail, na.rm = T), sum(dat_2013$newrel_fail, na.rm = T), sum(dat_2014$newrel_fail, na.rm = T), sum(dat_2015$newrel_fail, na.rm = T), sum(dat_2016$newrel_fail, na.rm = T), sum(dat_2017$newrel_fail, na.rm = T), sum(dat_2018$newrel_fail, na.rm = T), sum(dat_2019$newrel_fail, na.rm = T), sum(dat_2020$newrel_fail, na.rm = T), sum(dat_2021$newrel_fail, na.rm = T)),
...
analysis_pred <- pred_multi_sp_boot

analysis_pred <- analysis_pred %>% 
  mutate(obs_success=dat$newrel_succ,
         obs_fail=dat$newrel_fail,
         obs_died=dat$newrel_died,
         obs_lost=dat$newrel_lost,
         total_cases=obs_success+obs_fail+obs_died + obs_lost,
         exp_success=round(total_cases*success),
         exp_fail=round(total_cases*fail),
         exp_died=round(total_cases*died),
         exp_lost=round(total_cases*lost)
  )
analysis_pred$country <- dat$country
analysis_pred$WHO_region <- dat$g_whoregion

analysis_pred_2 <- analysis_pred %>%
  filter(year==2020 | year==2021) %>%
  select(country, WHO_region, year, obs_success, exp_success, obs_fail, exp_fail, obs_died, exp_died, obs_lost, exp_lost)

write.table(analysis_pred_2, file = "absoluteNum.csv", sep = ",", row.names = FALSE)  

analysis_pred_2020 <- analysis_pred_2 %>%
  filter(year==2020) %>%
  group_by(WHO_region) %>%
  summarise(sum_obs_success = sum(obs_success, na.rm = T),
            sum_exp_success = sum(exp_success, na.rm = T),
            sum_obs_fail = sum(obs_fail, na.rm = T),
            sum_exp_fail = sum(exp_fail, na.rm = T),
            sum_obs_died = sum(obs_died, na.rm = T),
            sum_exp_died = sum(exp_died, na.rm = T),
            sum_obs_lost = sum(obs_lost, na.rm = T),
            sum_exp_lost = sum(exp_lost, na.rm = T))

write.table(analysis_pred_2020, file = "absoluteNum_2020.csv", sep = ",", row.names = FALSE)  

analysis_pred_2021 <- analysis_pred_2 %>%
  filter(year==2021) %>%
  group_by(WHO_region) %>%
  summarise(sum_obs_success = sum(obs_success, na.rm = T),
            sum_exp_success = sum(exp_success, na.rm = T),
...
write.table(analysis_pred_2021, file = "absoluteNum_2021.csv", sep = ",", row.names = FALSE)  
...
ggsave("Exp_Obs_splines_boot_df3_2020_20240604.jpeg",g,device="jpeg",width=38,height=30,units=c("cm"), dpi = 300)
...
ggsave("Exp_Obs_splines_boot_df3_2021_20240604.jpeg",g_2021,device="jpeg",width=38,height=30,units=c("cm"), dpi = 300)
